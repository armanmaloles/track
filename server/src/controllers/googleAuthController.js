  ...
  try {
    ...
    if (!registration_token || !account_code) {
      return res.status(400).json({
        ok: false,
        message: "Registration token and account code are required.",
      });
    }
    ...
    try {
      ...
      return res
        .status(401)
        .json({ ok: false, message: "Registration token expired or invalid." });
    }
    if (decoded.purpose !== "google-registration") {
      return res
        .status(400)
        .json({ ok: false, message: "Invalid registration token." });
    }
    ...
    if (!code) {
      return res
        .status(400)
        .json({ ok: false, message: "Invalid account code." });
    }
    if (code.status === "used") {
      return res
        .status(400)
        .json({ ok: false, message: "Account code already used." });
    }
    if (code.expires_at && code.expires_at < new Date()) {
      return res
        .status(400)
        .json({ ok: false, message: "Account code has expired." });
    }
    if (code.is_admin) {
      return res
        .status(400)
        .json({ ok: false, message: "This code is for admin accounts only." });
    }
